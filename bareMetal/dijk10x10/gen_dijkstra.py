...
def createDivider():
	...
	for x in range(num_tasks-1):
		dividerFile.write("\tSend(msg, dijkstra_"+str(x)+");\n")
	...
	for x in range(num_tasks-1):
		dividerFile.write("\t\tmsg = Receive(); //MADAmsg, dijkstra_"+str(x)+");\n")
...
def createDijkstra():
	...
	dijkstraFile.write("\tSend(msg, divider);\n")
	dijkstraFile.write("}\n")
	...
	dijkstraFile.write("\tmsg = Receive(); //MADAmsg, divider);\n")
	...
	dijkstraFile.write('\tprintf("Dijkstra_'+str(IdTask)+' finished.");\n')
...
try:
	...
	dividerFile = open("divider.c", "w")
	createDivider();
	dividerFile.close()

	for IdTask in range(num_tasks-1):
		dijkstraFile = open("dijkstra_"+str(IdTask)+".c", "w")
		createDijkstra();
		dijkstraFile.close()
