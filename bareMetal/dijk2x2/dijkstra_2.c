...
void msgPath(NODE *rgnNodes, int chNode){
	if ((rgnNodes+chNode)->iPrev != NONE){
		msgPath(rgnNodes, (rgnNodes+chNode)->iPrev);
	}
	resultmsg[paths] = chNode+1;
	paths++;
}

void msgResult(int myID,int chStart, int chEnd){
	paths = 3;
	int k;
	for(k=0; k<33; k++)
		resultmsg[k] = 0;
	resultmsg[0] = chStart;
	resultmsg[1] = chEnd;
	resultmsg[2] = rgnNodes[myID][chEnd].iDist;
	msgPath(rgnNodes[myID], chEnd);

	Message msg;
	msg.length = 33;
	msg.msg[0] = -1;
	for(k=0; k<33; k++)
		msg.msg[k] = resultmsg[k];
	Send(msg, divider);
}
