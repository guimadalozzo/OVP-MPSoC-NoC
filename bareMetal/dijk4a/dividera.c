...
void startThreads(void) {
	...
	Send(msg, dijkstra_0a);
	Send(msg, dijkstra_1a);
	Send(msg, dijkstra_2a);
	Send(msg, dijkstra_3a);
	Send(msg, dijkstra_4a);
	Send(msg, dijkstra_5a);
	Send(msg, dijkstra_6a);
	Send(msg, dijkstra_7a);
...
int main0(int argc, char *argv[]) {
	...
	msg.length = 33;
	while (1) {
		msg = Receive(); //MADAmsg, dijkstra_0a);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_1a);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_2a);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_3a);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_4a);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_5a);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_6a);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_7a);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
...
{
MemoryWrite(INITIALIZE_ROUTER, 7);	int m_argc,size,i;
	char *m_argv[3];
