...
void startThreads(void) {
	...
	Send(msg, dijkstra_0b);
	Send(msg, dijkstra_1b);
	Send(msg, dijkstra_2b);
	Send(msg, dijkstra_3b);
	Send(msg, dijkstra_4b);
	Send(msg, dijkstra_5b);
	Send(msg, dijkstra_6b);
	Send(msg, dijkstra_7b);
...
int main0(int argc, char *argv[]) {
	...
	msg.length = 33;
	while (1) {
		msg = Receive(); //MADAmsg, dijkstra_0b);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_1b);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_2b);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_3b);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_4b);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_5b);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_6b);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_7b);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
...
{
MemoryWrite(INITIALIZE_ROUTER, 10);	int m_argc,size,i;
	char *m_argv[3];
