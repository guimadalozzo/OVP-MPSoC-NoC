...
void startThreads(void) {
	...
	Send(msg, dijkstra_0c);
	Send(msg, dijkstra_1c);
	Send(msg, dijkstra_2c);
	Send(msg, dijkstra_3c);
	Send(msg, dijkstra_4c);
	Send(msg, dijkstra_5c);
	Send(msg, dijkstra_6c);
	Send(msg, dijkstra_7c);
...
int main0(int argc, char *argv[]) {
	...
	msg.length = 33;
	while (1) {
		msg = Receive(); //MADAmsg, dijkstra_0c);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_1c);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_2c);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_3c);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_4c);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_5c);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_6c);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_7c);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
...
{
MemoryWrite(INITIALIZE_ROUTER, 25);	int m_argc,size,i;
	char *m_argv[3];
