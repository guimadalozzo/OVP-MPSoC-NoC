...
void startThreads(void) {
	...
	Send(msg, dijkstra_0d);
	Send(msg, dijkstra_1d);
	Send(msg, dijkstra_2d);
	Send(msg, dijkstra_3d);
	Send(msg, dijkstra_4d);
	Send(msg, dijkstra_5d);
	Send(msg, dijkstra_6d);
	Send(msg, dijkstra_7d);
...
int main0(int argc, char *argv[]) {
	...
	msg.length = 33;
	while (1) {
		msg = Receive(); //MADAmsg, dijkstra_0d);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_1d);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_2d);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_3d);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_4d);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_5d);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_6d);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
		msg = Receive(); //MADAmsg, dijkstra_7d);
		for (k=0; k<33; k++)
			result[k] = msg.msg[k];
		ProcessMessage();
...
{
MemoryWrite(INITIALIZE_ROUTER, 28);	int m_argc,size,i;
	char *m_argv[3];
