...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 35);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_17: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_17 %d", logN);
		...
		printf("Recebeu imag_17");
		printf("Inicio da FFT_17");

		FFT(logN, real, im);

		printf("Fim da FFT_17");
		...
		printf("Envio completo do Frame_17");
	}

	printf("Fim da Aplicacao FFT_17");
	MemoryWrite(END_SIM, 35);
