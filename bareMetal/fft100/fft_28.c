...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 57);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_28: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_28 %d", logN);
		...
		printf("Recebeu imag_28");
		printf("Inicio da FFT_28");

		FFT(logN, real, im);

		printf("Fim da FFT_28");
		...
		printf("Envio completo do Frame_28");
	}

	printf("Fim da Aplicacao FFT_28");
	MemoryWrite(END_SIM, 57);
