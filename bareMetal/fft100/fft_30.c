...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 61);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_30: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_30 %d", logN);
		...
		printf("Recebeu imag_30");
		printf("Inicio da FFT_30");

		FFT(logN, real, im);

		printf("Fim da FFT_30");
		...
		printf("Envio completo do Frame_30");
	}

	printf("Fim da Aplicacao FFT_30");
	MemoryWrite(END_SIM, 61);
