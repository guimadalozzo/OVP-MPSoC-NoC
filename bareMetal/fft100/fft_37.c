...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 75);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_37: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_37 %d", logN);
		...
		printf("Recebeu imag_37");
		printf("Inicio da FFT_37");

		FFT(logN, real, im);

		printf("Fim da FFT_37");
		...
		printf("Envio completo do Frame_37");
	}

	printf("Fim da Aplicacao FFT_37");
