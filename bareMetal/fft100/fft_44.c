...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 89);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_44: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_44 %d", logN);
		...
		printf("Recebeu imag_44");
		printf("Inicio da FFT_44");

		FFT(logN, real, im);

		printf("Fim da FFT_44");
		...
		printf("Envio completo do Frame_44");
	}

	printf("Fim da Aplicacao FFT_44");
	MemoryWrite(END_SIM, 89);
