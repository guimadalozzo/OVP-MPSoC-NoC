...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 93);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_46: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_46 %d", logN);
		...
		printf("Recebeu imag_46");
		printf("Inicio da FFT_46");

		FFT(logN, real, im);

		printf("Fim da FFT_46");
		...
		printf("Envio completo do Frame_46");
	}

	printf("Fim da Aplicacao FFT_46");
	MemoryWrite(END_SIM, 93);
