...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 17);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_8: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_8 %d", logN);
		...
		printf("Recebeu imag_8");
		printf("Inicio da FFT_8");
		...
		printf("Envio completo do Frame_8");
	}

	printf("Fim da Aplicacao FFT_8");
	MemoryWrite(END_SIM, 17);
