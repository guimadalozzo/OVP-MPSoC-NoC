...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 19);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_9: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_9 %d", logN);
		...
		printf("Recebeu imag_9");
		printf("Inicio da FFT_9");
		...
		printf("Envio completo do Frame_9");
	}

	printf("Fim da Aplicacao FFT_9");
	MemoryWrite(END_SIM, 19);
