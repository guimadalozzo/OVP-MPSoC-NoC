...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 205);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_102: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_102 %d", logN);
		...
		printf("Recebeu imag_102");
		printf("Inicio da FFT_102");

		FFT(logN, real, im);

		printf("Fim da FFT_102");
		...
		printf("Envio completo do Frame_102");
	}

	printf("Fim da Aplicacao FFT_102");
	MemoryWrite(END_SIM, 205);
