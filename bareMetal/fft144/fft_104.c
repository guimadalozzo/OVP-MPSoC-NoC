...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 209);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_104: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_104 %d", logN);
		...
		printf("Recebeu imag_104");
		printf("Inicio da FFT_104");

		FFT(logN, real, im);

		printf("Fim da FFT_104");
		...
		printf("Envio completo do Frame_104");
	}

	printf("Fim da Aplicacao FFT_104");
	MemoryWrite(END_SIM, 209);
