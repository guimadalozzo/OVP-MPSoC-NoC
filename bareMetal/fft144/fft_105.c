...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 211);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_105: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_105 %d", logN);
		...
		printf("Recebeu imag_105");
		printf("Inicio da FFT_105");

		FFT(logN, real, im);

		printf("Fim da FFT_105");
		...
		printf("Envio completo do Frame_105");
	}

	printf("Fim da Aplicacao FFT_105");
	MemoryWrite(END_SIM, 211);
