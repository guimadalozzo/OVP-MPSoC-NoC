...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 221);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_110: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_110 %d", logN);
		...
		printf("Recebeu imag_110");
		printf("Inicio da FFT_110");

		FFT(logN, real, im);

		printf("Fim da FFT_110");
		...
		printf("Envio completo do Frame_110");
	}

	printf("Fim da Aplicacao FFT_110");
	MemoryWrite(END_SIM, 221);
