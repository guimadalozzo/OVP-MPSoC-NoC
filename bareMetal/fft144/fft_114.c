...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 229);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_114: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_114 %d", logN);
		...
		printf("Recebeu imag_114");
		printf("Inicio da FFT_114");

		FFT(logN, real, im);

		printf("Fim da FFT_114");
		...
		printf("Envio completo do Frame_114");
	}

	printf("Fim da Aplicacao FFT_114");
	MemoryWrite(END_SIM, 229);
