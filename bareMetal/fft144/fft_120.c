...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 241);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_120: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_120 %d", logN);
		...
		printf("Recebeu imag_120");
		printf("Inicio da FFT_120");

		FFT(logN, real, im);

		printf("Fim da FFT_120");
		...
		printf("Envio completo do Frame_120");
	}

	printf("Fim da Aplicacao FFT_120");
	MemoryWrite(END_SIM, 241);
