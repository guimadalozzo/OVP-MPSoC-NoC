...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 251);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_125: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_125 %d", logN);
		...
		printf("Recebeu imag_125");
		printf("Inicio da FFT_125");

		FFT(logN, real, im);

		printf("Fim da FFT_125");
		...
		printf("Envio completo do Frame_125");
	}

	printf("Fim da Aplicacao FFT_125");
	MemoryWrite(END_SIM, 251);
