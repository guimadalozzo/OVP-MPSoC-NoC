...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 253);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_126: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_126 %d", logN);
		...
		printf("Recebeu imag_126");
		printf("Inicio da FFT_126");

		FFT(logN, real, im);

		printf("Fim da FFT_126");
		...
		printf("Envio completo do Frame_126");
	}

	printf("Fim da Aplicacao FFT_126");
	MemoryWrite(END_SIM, 253);
