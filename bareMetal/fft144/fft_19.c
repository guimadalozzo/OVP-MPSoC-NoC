...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 39);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_19: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_19 %d", logN);
		...
		printf("Recebeu imag_19");
		printf("Inicio da FFT_19");

		FFT(logN, real, im);

		printf("Fim da FFT_19");
		...
		printf("Envio completo do Frame_19");
	}

	printf("Fim da Aplicacao FFT_19");
	MemoryWrite(END_SIM, 39);
