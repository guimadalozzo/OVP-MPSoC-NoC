...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 5);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_2: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_2 %d", logN);
		...
		printf("Recebeu imag_2");
		printf("Inicio da FFT_2");
		...
		printf("Envio completo do Frame_2");
	}

	printf("Fim da Aplicacao FFT_2");
	MemoryWrite(END_SIM, 5);
