...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 41);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_20: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_20 %d", logN);
		...
		printf("Recebeu imag_20");
		printf("Inicio da FFT_20");

		FFT(logN, real, im);

		printf("Fim da FFT_20");
		...
		printf("Envio completo do Frame_20");
	}

	printf("Fim da Aplicacao FFT_20");
	MemoryWrite(END_SIM, 41);
