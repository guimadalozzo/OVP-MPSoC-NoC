...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 77);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_38: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_38 %d", logN);
		...
		printf("Recebeu imag_38");
		printf("Inicio da FFT_38");

		FFT(logN, real, im);

		printf("Fim da FFT_38");
		...
		printf("Envio completo do Frame_38");
	}

	printf("Fim da Aplicacao FFT_38");
	MemoryWrite(END_SIM, 77);
