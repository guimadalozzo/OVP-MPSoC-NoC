...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 95);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_47: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_47 %d", logN);
		...
		printf("Recebeu imag_47");
		printf("Inicio da FFT_47");

		FFT(logN, real, im);

		printf("Fim da FFT_47");
		...
		printf("Envio completo do Frame_47");
	}

	printf("Fim da Aplicacao FFT_47");
	MemoryWrite(END_SIM, 95);
