...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 97);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_48: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_48 %d", logN);
		...
		printf("Recebeu imag_48");
		printf("Inicio da FFT_48");

		FFT(logN, real, im);

		printf("Fim da FFT_48");
		...
		printf("Envio completo do Frame_48");
	}

	printf("Fim da Aplicacao FFT_48");
	MemoryWrite(END_SIM, 97);
