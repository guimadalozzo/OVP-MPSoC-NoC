...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 101);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_50: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_50 %d", logN);
		...
		printf("Recebeu imag_50");
		printf("Inicio da FFT_50");

		FFT(logN, real, im);

		printf("Fim da FFT_50");
		...
		printf("Envio completo do Frame_50");
	}

	printf("Fim da Aplicacao FFT_50");
	MemoryWrite(END_SIM, 101);
