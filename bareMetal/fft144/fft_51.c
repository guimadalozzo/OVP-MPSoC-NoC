...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 103);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_51: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_51 %d", logN);
		...
		printf("Recebeu imag_51");
		printf("Inicio da FFT_51");

		FFT(logN, real, im);

		printf("Fim da FFT_51");
		...
		printf("Envio completo do Frame_51");
	}

	printf("Fim da Aplicacao FFT_51");
	MemoryWrite(END_SIM, 103);
