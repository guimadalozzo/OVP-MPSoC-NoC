...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 105);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_52: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_52 %d", logN);
		...
		printf("Recebeu imag_52");
		printf("Inicio da FFT_52");

		FFT(logN, real, im);

		printf("Fim da FFT_52");
		...
		printf("Envio completo do Frame_52");
	}

	printf("Fim da Aplicacao FFT_52");
	MemoryWrite(END_SIM, 105);
