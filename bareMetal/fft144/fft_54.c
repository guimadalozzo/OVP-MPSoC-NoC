...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 109);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_54: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_54 %d", logN);
		...
		printf("Recebeu imag_54");
		printf("Inicio da FFT_54");

		FFT(logN, real, im);

		printf("Fim da FFT_54");
		...
		printf("Envio completo do Frame_54");
	}

	printf("Fim da Aplicacao FFT_54");
	MemoryWrite(END_SIM, 109);
