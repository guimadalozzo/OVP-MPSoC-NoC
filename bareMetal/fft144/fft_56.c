...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 113);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_56: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_56 %d", logN);
		...
		printf("Recebeu imag_56");
		printf("Inicio da FFT_56");

		FFT(logN, real, im);

		printf("Fim da FFT_56");
		...
		printf("Envio completo do Frame_56");
	}

	printf("Fim da Aplicacao FFT_56");
	MemoryWrite(END_SIM, 113);
