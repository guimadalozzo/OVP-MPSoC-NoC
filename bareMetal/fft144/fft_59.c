...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 119);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_59: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_59 %d", logN);
		...
		printf("Recebeu imag_59");
		printf("Inicio da FFT_59");

		FFT(logN, real, im);

		printf("Fim da FFT_59");
		...
		printf("Envio completo do Frame_59");
	}

	printf("Fim da Aplicacao FFT_59");
	MemoryWrite(END_SIM, 119);
