...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 123);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_61: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_61 %d", logN);
		...
		printf("Recebeu imag_61");
		printf("Inicio da FFT_61");

		FFT(logN, real, im);

		printf("Fim da FFT_61");
		...
		printf("Envio completo do Frame_61");
	}

	printf("Fim da Aplicacao FFT_61");
	MemoryWrite(END_SIM, 123);
