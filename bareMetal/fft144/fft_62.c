...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 125);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_62: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_62 %d", logN);
		...
		printf("Recebeu imag_62");
		printf("Inicio da FFT_62");

		FFT(logN, real, im);

		printf("Fim da FFT_62");
		...
		printf("Envio completo do Frame_62");
	}

	printf("Fim da Aplicacao FFT_62");
	MemoryWrite(END_SIM, 125);
