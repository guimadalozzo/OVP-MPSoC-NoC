...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 127);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_63: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_63 %d", logN);
		...
		printf("Recebeu imag_63");
		printf("Inicio da FFT_63");

		FFT(logN, real, im);

		printf("Fim da FFT_63");
		...
		printf("Envio completo do Frame_63");
	}

	printf("Fim da Aplicacao FFT_63");
	MemoryWrite(END_SIM, 127);
