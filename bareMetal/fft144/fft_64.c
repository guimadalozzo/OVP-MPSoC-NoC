...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 129);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_64: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_64 %d", logN);
		...
		printf("Recebeu imag_64");
		printf("Inicio da FFT_64");

		FFT(logN, real, im);

		printf("Fim da FFT_64");
		...
		printf("Envio completo do Frame_64");
	}

	printf("Fim da Aplicacao FFT_64");
	MemoryWrite(END_SIM, 129);
