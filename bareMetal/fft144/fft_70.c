...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 141);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_70: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_70 %d", logN);
		...
		printf("Recebeu imag_70");
		printf("Inicio da FFT_70");

		FFT(logN, real, im);

		printf("Fim da FFT_70");
		...
		printf("Envio completo do Frame_70");
	}

	printf("Fim da Aplicacao FFT_70");
	MemoryWrite(END_SIM, 141);
