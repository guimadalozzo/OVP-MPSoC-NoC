...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 143);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_71: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_71 %d", logN);
		...
		printf("Recebeu imag_71");
		printf("Inicio da FFT_71");

		FFT(logN, real, im);

		printf("Fim da FFT_71");
		...
		printf("Envio completo do Frame_71");
	}

	printf("Fim da Aplicacao FFT_71");
	MemoryWrite(END_SIM, 143);
