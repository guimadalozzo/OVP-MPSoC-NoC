...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 145);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_72: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_72 %d", logN);
		...
		printf("Recebeu imag_72");
		printf("Inicio da FFT_72");

		FFT(logN, real, im);

		printf("Fim da FFT_72");
		...
		printf("Envio completo do Frame_72");
	}

	printf("Fim da Aplicacao FFT_72");
	MemoryWrite(END_SIM, 145);
