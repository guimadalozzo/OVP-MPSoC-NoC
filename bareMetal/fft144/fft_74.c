...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 149);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_74: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_74 %d", logN);
		...
		printf("Recebeu imag_74");
		printf("Inicio da FFT_74");

		FFT(logN, real, im);

		printf("Fim da FFT_74");
		...
		printf("Envio completo do Frame_74");
	}

	printf("Fim da Aplicacao FFT_74");
	MemoryWrite(END_SIM, 149);
