...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 155);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_77: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_77 %d", logN);
		...
		printf("Recebeu imag_77");
		printf("Inicio da FFT_77");

		FFT(logN, real, im);

		printf("Fim da FFT_77");
		...
		printf("Envio completo do Frame_77");
	}

	printf("Fim da Aplicacao FFT_77");
	MemoryWrite(END_SIM, 155);
