...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 161);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_80: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_80 %d", logN);
		...
		printf("Recebeu imag_80");
		printf("Inicio da FFT_80");

		FFT(logN, real, im);

		printf("Fim da FFT_80");
		...
		printf("Envio completo do Frame_80");
	}

	printf("Fim da Aplicacao FFT_80");
	MemoryWrite(END_SIM, 161);
