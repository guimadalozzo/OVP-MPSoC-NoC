...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 171);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_85: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_85 %d", logN);
		...
		printf("Recebeu imag_85");
		printf("Inicio da FFT_85");

		FFT(logN, real, im);

		printf("Fim da FFT_85");
		...
		printf("Envio completo do Frame_85");
	}

	printf("Fim da Aplicacao FFT_85");
	MemoryWrite(END_SIM, 171);
