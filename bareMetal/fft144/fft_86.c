...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 173);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_86: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_86 %d", logN);
		...
		printf("Recebeu imag_86");
		printf("Inicio da FFT_86");

		FFT(logN, real, im);

		printf("Fim da FFT_86");
		...
		printf("Envio completo do Frame_86");
	}

	printf("Fim da Aplicacao FFT_86");
	MemoryWrite(END_SIM, 173);
