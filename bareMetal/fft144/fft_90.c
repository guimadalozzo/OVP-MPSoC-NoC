...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 181);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_90: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_90 %d", logN);
		...
		printf("Recebeu imag_90");
		printf("Inicio da FFT_90");

		FFT(logN, real, im);

		printf("Fim da FFT_90");
		...
		printf("Envio completo do Frame_90");
	}

	printf("Fim da Aplicacao FFT_90");
	MemoryWrite(END_SIM, 181);
