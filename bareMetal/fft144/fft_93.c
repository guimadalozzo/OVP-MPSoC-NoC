...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 187);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_93: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_93 %d", logN);
		...
		printf("Recebeu imag_93");
		printf("Inicio da FFT_93");

		FFT(logN, real, im);

		printf("Fim da FFT_93");
		...
		printf("Envio completo do Frame_93");
	}

	printf("Fim da Aplicacao FFT_93");
	MemoryWrite(END_SIM, 187);
