...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 199);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_99: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_99 %d", logN);
		...
		printf("Recebeu imag_99");
		printf("Inicio da FFT_99");

		FFT(logN, real, im);

		printf("Fim da FFT_99");
		...
		printf("Envio completo do Frame_99");
	}

	printf("Fim da Aplicacao FFT_99");
	MemoryWrite(END_SIM, 199);
