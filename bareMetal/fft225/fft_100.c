...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 201);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_100: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_100 %d", logN);
		...
		printf("Recebeu imag_100");
		printf("Inicio da FFT_100");

		FFT(logN, real, im);

		printf("Fim da FFT_100");
		...
		printf("Envio completo do Frame_100");
	}

	printf("Fim da Aplicacao FFT_100");
	MemoryWrite(END_SIM, 201);
