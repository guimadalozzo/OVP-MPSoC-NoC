...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 37);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_18: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_18 %d", logN);
		...
		printf("Recebeu imag_18");
		printf("Inicio da FFT_18");

		FFT(logN, real, im);

		printf("Fim da FFT_18");
		...
		printf("Envio completo do Frame_18");
	}

	printf("Fim da Aplicacao FFT_18");
	MemoryWrite(END_SIM, 37);
