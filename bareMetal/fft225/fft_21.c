...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 43);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_21: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_21 %d", logN);
		...
		printf("Recebeu imag_21");
		printf("Inicio da FFT_21");

		FFT(logN, real, im);

		printf("Fim da FFT_21");
		...
		printf("Envio completo do Frame_21");
	}

	printf("Fim da Aplicacao FFT_21");
	MemoryWrite(END_SIM, 43);
