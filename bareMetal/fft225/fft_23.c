...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 47);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_23: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_23 %d", logN);
		...
		printf("Recebeu imag_23");
		printf("Inicio da FFT_23");

		FFT(logN, real, im);

		printf("Fim da FFT_23");
		...
		printf("Envio completo do Frame_23");
	}

	printf("Fim da Aplicacao FFT_23");
	MemoryWrite(END_SIM, 47);
