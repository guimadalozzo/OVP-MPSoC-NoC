...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 51);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_25: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_25 %d", logN);
		...
		printf("Recebeu imag_25");
		printf("Inicio da FFT_25");

		FFT(logN, real, im);

		printf("Fim da FFT_25");
		...
		printf("Envio completo do Frame_25");
	}

	printf("Fim da Aplicacao FFT_25");
	MemoryWrite(END_SIM, 51);
