...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 55);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_27: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_27 %d", logN);
		...
		printf("Recebeu imag_27");
		printf("Inicio da FFT_27");

		FFT(logN, real, im);

		printf("Fim da FFT_27");
		...
		printf("Envio completo do Frame_27");
	}

	printf("Fim da Aplicacao FFT_27");
	MemoryWrite(END_SIM, 55);
