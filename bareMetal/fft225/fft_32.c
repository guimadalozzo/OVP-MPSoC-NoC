...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 65);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_32: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_32 %d", logN);
		...
		printf("Recebeu imag_32");
		printf("Inicio da FFT_32");

		FFT(logN, real, im);

		printf("Fim da FFT_32");
		...
		printf("Envio completo do Frame_32");
	}

	printf("Fim da Aplicacao FFT_32");
	MemoryWrite(END_SIM, 65);
