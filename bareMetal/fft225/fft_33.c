...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 67);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_33: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_33 %d", logN);
		...
		printf("Recebeu imag_33");
		printf("Inicio da FFT_33");

		FFT(logN, real, im);

		printf("Fim da FFT_33");
		...
		printf("Envio completo do Frame_33");
	}

	printf("Fim da Aplicacao FFT_33");
	MemoryWrite(END_SIM, 67);
