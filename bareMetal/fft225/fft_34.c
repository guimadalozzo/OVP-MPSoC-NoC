...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 69);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_34: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_34 %d", logN);
		...
		printf("Recebeu imag_34");
		printf("Inicio da FFT_34");

		FFT(logN, real, im);

		printf("Fim da FFT_34");
		...
		printf("Envio completo do Frame_34");
	}

	printf("Fim da Aplicacao FFT_34");
	MemoryWrite(END_SIM, 69);
