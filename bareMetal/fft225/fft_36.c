...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 73);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_36: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_36 %d", logN);
		...
		printf("Recebeu imag_36");
		printf("Inicio da FFT_36");

		FFT(logN, real, im);

		printf("Fim da FFT_36");
		...
		printf("Envio completo do Frame_36");
	}

	printf("Fim da Aplicacao FFT_36");
	MemoryWrite(END_SIM, 73);
