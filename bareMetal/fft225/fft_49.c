...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 99);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_49: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_49 %d", logN);
		...
		printf("Recebeu imag_49");
		printf("Inicio da FFT_49");

		FFT(logN, real, im);

		printf("Fim da FFT_49");
		...
		printf("Envio completo do Frame_49");
	}

	printf("Fim da Aplicacao FFT_49");
	MemoryWrite(END_SIM, 99);
