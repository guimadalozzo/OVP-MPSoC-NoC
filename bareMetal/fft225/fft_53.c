...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 107);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_53: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_53 %d", logN);
		...
		printf("Recebeu imag_53");
		printf("Inicio da FFT_53");

		FFT(logN, real, im);

		printf("Fim da FFT_53");
		...
		printf("Envio completo do Frame_53");
	}

	printf("Fim da Aplicacao FFT_53");
	MemoryWrite(END_SIM, 107);
