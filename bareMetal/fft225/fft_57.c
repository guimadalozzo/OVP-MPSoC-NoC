...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 115);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_57: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_57 %d", logN);
		...
		printf("Recebeu imag_57");
		printf("Inicio da FFT_57");

		FFT(logN, real, im);

		printf("Fim da FFT_57");
		...
		printf("Envio completo do Frame_57");
	}

	printf("Fim da Aplicacao FFT_57");
	MemoryWrite(END_SIM, 115);
