...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 117);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_58: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_58 %d", logN);
		...
		printf("Recebeu imag_58");
		printf("Inicio da FFT_58");

		FFT(logN, real, im);

		printf("Fim da FFT_58");
		...
		printf("Envio completo do Frame_58");
	}

	printf("Fim da Aplicacao FFT_58");
	MemoryWrite(END_SIM, 117);
