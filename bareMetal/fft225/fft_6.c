...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 13);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_6: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_6 %d", logN);
		...
		printf("Recebeu imag_6");
		printf("Inicio da FFT_6");
		...
		printf("Envio completo do Frame_6");
	}

	printf("Fim da Aplicacao FFT_6");
	MemoryWrite(END_SIM, 13);
