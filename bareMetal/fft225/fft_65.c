...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 131);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_65: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_65 %d", logN);
		...
		printf("Recebeu imag_65");
		printf("Inicio da FFT_65");

		FFT(logN, real, im);

		printf("Fim da FFT_65");
		...
		printf("Envio completo do Frame_65");
	}

	printf("Fim da Aplicacao FFT_65");
	MemoryWrite(END_SIM, 131);
