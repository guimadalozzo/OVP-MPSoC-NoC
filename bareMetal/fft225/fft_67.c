...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 135);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_67: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_67 %d", logN);
		...
		printf("Recebeu imag_67");
		printf("Inicio da FFT_67");

		FFT(logN, real, im);

		printf("Fim da FFT_67");
		...
		printf("Envio completo do Frame_67");
	}

	printf("Fim da Aplicacao FFT_67");
	MemoryWrite(END_SIM, 135);
