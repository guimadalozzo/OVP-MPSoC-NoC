...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 139);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_69: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_69 %d", logN);
		...
		printf("Recebeu imag_69");
		printf("Inicio da FFT_69");

		FFT(logN, real, im);

		printf("Fim da FFT_69");
		...
		printf("Envio completo do Frame_69");
	}

	printf("Fim da Aplicacao FFT_69");
	MemoryWrite(END_SIM, 139);
