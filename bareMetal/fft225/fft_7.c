...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 15);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_7: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_7 %d", logN);
		...
		printf("Recebeu imag_7");
		printf("Inicio da FFT_7");
		...
		printf("Envio completo do Frame_7");
	}

	printf("Fim da Aplicacao FFT_7");
	MemoryWrite(END_SIM, 15);
