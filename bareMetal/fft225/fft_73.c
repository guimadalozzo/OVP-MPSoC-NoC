...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 147);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_73: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_73 %d", logN);
		...
		printf("Recebeu imag_73");
		printf("Inicio da FFT_73");

		FFT(logN, real, im);

		printf("Fim da FFT_73");
		...
		printf("Envio completo do Frame_73");
	}

	printf("Fim da Aplicacao FFT_73");
	MemoryWrite(END_SIM, 147);
