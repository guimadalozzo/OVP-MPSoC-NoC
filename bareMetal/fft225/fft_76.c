...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 153);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_76: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_76 %d", logN);
		...
		printf("Recebeu imag_76");
		printf("Inicio da FFT_76");

		FFT(logN, real, im);

		printf("Fim da FFT_76");
		...
		printf("Envio completo do Frame_76");
	}

	printf("Fim da Aplicacao FFT_76");
	MemoryWrite(END_SIM, 153);
