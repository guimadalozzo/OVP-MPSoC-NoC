...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 159);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_79: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_79 %d", logN);
		...
		printf("Recebeu imag_79");
		printf("Inicio da FFT_79");

		FFT(logN, real, im);

		printf("Fim da FFT_79");
		...
		printf("Envio completo do Frame_79");
	}

	printf("Fim da Aplicacao FFT_79");
	MemoryWrite(END_SIM, 159);
