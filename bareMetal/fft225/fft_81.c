...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 163);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_81: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_81 %d", logN);
		...
		printf("Recebeu imag_81");
		printf("Inicio da FFT_81");

		FFT(logN, real, im);

		printf("Fim da FFT_81");
		...
		printf("Envio completo do Frame_81");
	}

	printf("Fim da Aplicacao FFT_81");
	MemoryWrite(END_SIM, 163);
