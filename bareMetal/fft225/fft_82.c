...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 165);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_82: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_82 %d", logN);
		...
		printf("Recebeu imag_82");
		printf("Inicio da FFT_82");

		FFT(logN, real, im);

		printf("Fim da FFT_82");
		...
		printf("Envio completo do Frame_82");
	}

	printf("Fim da Aplicacao FFT_82");
	MemoryWrite(END_SIM, 165);
