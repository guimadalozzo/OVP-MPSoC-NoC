...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 169);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_84: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_84 %d", logN);
		...
		printf("Recebeu imag_84");
		printf("Inicio da FFT_84");

		FFT(logN, real, im);

		printf("Fim da FFT_84");
		...
		printf("Envio completo do Frame_84");
	}

	printf("Fim da Aplicacao FFT_84");
	MemoryWrite(END_SIM, 169);
