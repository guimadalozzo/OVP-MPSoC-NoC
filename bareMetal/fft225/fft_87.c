...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 175);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_87: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_87 %d", logN);
		...
		printf("Recebeu imag_87");
		printf("Inicio da FFT_87");

		FFT(logN, real, im);

		printf("Fim da FFT_87");
		...
		printf("Envio completo do Frame_87");
	}

	printf("Fim da Aplicacao FFT_87");
	MemoryWrite(END_SIM, 175);
