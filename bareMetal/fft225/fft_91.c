...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 183);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_91: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_91 %d", logN);
		...
		printf("Recebeu imag_91");
		printf("Inicio da FFT_91");

		FFT(logN, real, im);

		printf("Fim da FFT_91");
		...
		printf("Envio completo do Frame_91");
	}

	printf("Fim da Aplicacao FFT_91");
	MemoryWrite(END_SIM, 183);
