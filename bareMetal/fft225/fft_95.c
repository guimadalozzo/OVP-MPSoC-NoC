...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 191);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_95: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_95 %d", logN);
		...
		printf("Recebeu imag_95");
		printf("Inicio da FFT_95");

		FFT(logN, real, im);

		printf("Fim da FFT_95");
		...
		printf("Envio completo do Frame_95");
	}

	printf("Fim da Aplicacao FFT_95");
	MemoryWrite(END_SIM, 191);
