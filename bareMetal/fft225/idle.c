...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 224);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_0: %d", i++);

		//msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_0 %d", logN);
		...
		printf("Recebeu imag_0");
		printf("Inicio da FFT_0");
...
//		Send(msg, front_0);
		...
		printf("Envio completo do Frame_0");
	}

	printf("Fim da Aplicacao FFT_0");
	MemoryWrite(END_SIM, 224);
