...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 213);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_106: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_106 %d", logN);
		...
		printf("Recebeu imag_106");
		printf("Inicio da FFT_106");

		FFT(logN, real, im);

		printf("Fim da FFT_106");
		...
		printf("Envio completo do Frame_106");
	}

	printf("Fim da Aplicacao FFT_106");
	MemoryWrite(END_SIM, 213);
