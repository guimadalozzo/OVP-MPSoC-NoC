...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 217);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_108: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_108 %d", logN);
		...
		printf("Recebeu imag_108");
		printf("Inicio da FFT_108");

		FFT(logN, real, im);

		printf("Fim da FFT_108");
		...
		printf("Envio completo do Frame_108");
	}

	printf("Fim da Aplicacao FFT_108");
	MemoryWrite(END_SIM, 217);
