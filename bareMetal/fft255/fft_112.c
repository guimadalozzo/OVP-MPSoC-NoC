...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 225);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_112: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_112 %d", logN);
		...
		printf("Recebeu imag_112");
		printf("Inicio da FFT_112");

		FFT(logN, real, im);

		printf("Fim da FFT_112");
		...
		printf("Envio completo do Frame_112");
	}

	printf("Fim da Aplicacao FFT_112");
	MemoryWrite(END_SIM, 225);
