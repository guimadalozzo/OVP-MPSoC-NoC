...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 227);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_113: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_113 %d", logN);
		...
		printf("Recebeu imag_113");
		printf("Inicio da FFT_113");

		FFT(logN, real, im);

		printf("Fim da FFT_113");
		...
		printf("Envio completo do Frame_113");
	}

	printf("Fim da Aplicacao FFT_113");
	MemoryWrite(END_SIM, 227);
