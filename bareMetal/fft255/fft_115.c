...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 231);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_115: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_115 %d", logN);
		...
		printf("Recebeu imag_115");
		printf("Inicio da FFT_115");

		FFT(logN, real, im);

		printf("Fim da FFT_115");
		...
		printf("Envio completo do Frame_115");
	}

	printf("Fim da Aplicacao FFT_115");
	MemoryWrite(END_SIM, 231);
