...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 233);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_116: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_116 %d", logN);
		...
		printf("Recebeu imag_116");
		printf("Inicio da FFT_116");

		FFT(logN, real, im);

		printf("Fim da FFT_116");
		...
		printf("Envio completo do Frame_116");
	}

	printf("Fim da Aplicacao FFT_116");
	MemoryWrite(END_SIM, 233);
