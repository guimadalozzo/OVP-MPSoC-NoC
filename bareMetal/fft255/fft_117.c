...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 235);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_117: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_117 %d", logN);
		...
		printf("Recebeu imag_117");
		printf("Inicio da FFT_117");

		FFT(logN, real, im);

		printf("Fim da FFT_117");
		...
		printf("Envio completo do Frame_117");
	}

	printf("Fim da Aplicacao FFT_117");
	MemoryWrite(END_SIM, 235);
