...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 237);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_118: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_118 %d", logN);
		...
		printf("Recebeu imag_118");
		printf("Inicio da FFT_118");

		FFT(logN, real, im);

		printf("Fim da FFT_118");
		...
		printf("Envio completo do Frame_118");
	}

	printf("Fim da Aplicacao FFT_118");
	MemoryWrite(END_SIM, 237);
