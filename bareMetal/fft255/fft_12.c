...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 25);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_12: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_12 %d", logN);
		...
		printf("Recebeu imag_12");
		printf("Inicio da FFT_12");

		FFT(logN, real, im);

		printf("Fim da FFT_12");
		...
		printf("Envio completo do Frame_12");
	}

	printf("Fim da Aplicacao FFT_12");
