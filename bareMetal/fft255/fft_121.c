...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 243);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_121: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_121 %d", logN);
		...
		printf("Recebeu imag_121");
		printf("Inicio da FFT_121");

		FFT(logN, real, im);

		printf("Fim da FFT_121");
		...
		printf("Envio completo do Frame_121");
	}

	printf("Fim da Aplicacao FFT_121");
	MemoryWrite(END_SIM, 243);
