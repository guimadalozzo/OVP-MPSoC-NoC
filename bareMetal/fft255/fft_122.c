...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 245);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_122: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_122 %d", logN);
		...
		printf("Recebeu imag_122");
		printf("Inicio da FFT_122");

		FFT(logN, real, im);

		printf("Fim da FFT_122");
		...
		printf("Envio completo do Frame_122");
	}

	printf("Fim da Aplicacao FFT_122");
	MemoryWrite(END_SIM, 245);
