...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 247);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_123: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_123 %d", logN);
		...
		printf("Recebeu imag_123");
		printf("Inicio da FFT_123");

		FFT(logN, real, im);

		printf("Fim da FFT_123");
		...
		printf("Envio completo do Frame_123");
	}

	printf("Fim da Aplicacao FFT_123");
	MemoryWrite(END_SIM, 247);
