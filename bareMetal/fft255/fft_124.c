...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 249);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_124: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_124 %d", logN);
		...
		printf("Recebeu imag_124");
		printf("Inicio da FFT_124");

		FFT(logN, real, im);

		printf("Fim da FFT_124");
		...
		printf("Envio completo do Frame_124");
	}

	printf("Fim da Aplicacao FFT_124");
	MemoryWrite(END_SIM, 249);
