...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 31);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_15: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_15 %d", logN);
		...
		printf("Recebeu imag_15");
		printf("Inicio da FFT_15");

		FFT(logN, real, im);

		printf("Fim da FFT_15");
		...
		printf("Envio completo do Frame_15");
	}

	printf("Fim da Aplicacao FFT_15");
	MemoryWrite(END_SIM, 31);
