...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 53);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_26: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_26 %d", logN);
		...
		printf("Recebeu imag_26");
		printf("Inicio da FFT_26");

		FFT(logN, real, im);

		printf("Fim da FFT_26");
		...
		printf("Envio completo do Frame_26");
	}

	printf("Fim da Aplicacao FFT_26");
	MemoryWrite(END_SIM, 53);
