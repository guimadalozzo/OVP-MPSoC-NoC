...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 59);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_29: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_29 %d", logN);
		...
		printf("Recebeu imag_29");
		printf("Inicio da FFT_29");

		FFT(logN, real, im);

		printf("Fim da FFT_29");
		...
		printf("Envio completo do Frame_29");
	}

	printf("Fim da Aplicacao FFT_29");
	MemoryWrite(END_SIM, 59);
