...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 63);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_31: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_31 %d", logN);
		...
		printf("Recebeu imag_31");
		printf("Inicio da FFT_31");

		FFT(logN, real, im);

		printf("Fim da FFT_31");
		...
		printf("Envio completo do Frame_31");
	}

	printf("Fim da Aplicacao FFT_31");
	MemoryWrite(END_SIM, 63);
