...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 71);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_35: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_35 %d", logN);
		...
		printf("Recebeu imag_35");
		printf("Inicio da FFT_35");

		FFT(logN, real, im);

		printf("Fim da FFT_35");
		...
		printf("Envio completo do Frame_35");
	}

	printf("Fim da Aplicacao FFT_35");
	MemoryWrite(END_SIM, 71);
