...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 79);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_39: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_39 %d", logN);
		...
		printf("Recebeu imag_39");
		printf("Inicio da FFT_39");

		FFT(logN, real, im);

		printf("Fim da FFT_39");
		...
		printf("Envio completo do Frame_39");
	}

	printf("Fim da Aplicacao FFT_39");
	MemoryWrite(END_SIM, 79);
