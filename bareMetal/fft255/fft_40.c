...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 81);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_40: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_40 %d", logN);
		...
		printf("Recebeu imag_40");
		printf("Inicio da FFT_40");

		FFT(logN, real, im);

		printf("Fim da FFT_40");
		...
		printf("Envio completo do Frame_40");
	}

	printf("Fim da Aplicacao FFT_40");
	MemoryWrite(END_SIM, 81);
