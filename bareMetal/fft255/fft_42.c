...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 85);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_42: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_42 %d", logN);
		...
		printf("Recebeu imag_42");
		printf("Inicio da FFT_42");

		FFT(logN, real, im);

		printf("Fim da FFT_42");
		...
		printf("Envio completo do Frame_42");
	}

	printf("Fim da Aplicacao FFT_42");
	MemoryWrite(END_SIM, 85);
