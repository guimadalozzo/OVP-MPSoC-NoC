...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 87);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_43: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_43 %d", logN);
		...
		printf("Recebeu imag_43");
		printf("Inicio da FFT_43");

		FFT(logN, real, im);

		printf("Fim da FFT_43");
		...
		printf("Envio completo do Frame_43");
	}

	printf("Fim da Aplicacao FFT_43");
	MemoryWrite(END_SIM, 87);
