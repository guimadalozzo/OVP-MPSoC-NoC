...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 91);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_45: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_45 %d", logN);
		...
		printf("Recebeu imag_45");
		printf("Inicio da FFT_45");

		FFT(logN, real, im);

		printf("Fim da FFT_45");
		...
		printf("Envio completo do Frame_45");
	}

	printf("Fim da Aplicacao FFT_45");
	MemoryWrite(END_SIM, 91);
