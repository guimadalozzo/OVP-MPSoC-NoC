...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 111);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_55: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_55 %d", logN);
		...
		printf("Recebeu imag_55");
		printf("Inicio da FFT_55");

		FFT(logN, real, im);

		printf("Fim da FFT_55");
		...
		printf("Envio completo do Frame_55");
	}

	printf("Fim da Aplicacao FFT_55");
	MemoryWrite(END_SIM, 111);
