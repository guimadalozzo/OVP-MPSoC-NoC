...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 121);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_60: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_60 %d", logN);
		...
		printf("Recebeu imag_60");
		printf("Inicio da FFT_60");

		FFT(logN, real, im);

		printf("Fim da FFT_60");
		...
		printf("Envio completo do Frame_60");
	}

	printf("Fim da Aplicacao FFT_60");
	MemoryWrite(END_SIM, 121);
