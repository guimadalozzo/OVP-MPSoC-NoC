...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 133);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_66: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_66 %d", logN);
		...
		printf("Recebeu imag_66");
		printf("Inicio da FFT_66");

		FFT(logN, real, im);

		printf("Fim da FFT_66");
		...
		printf("Envio completo do Frame_66");
	}

	printf("Fim da Aplicacao FFT_66");
	MemoryWrite(END_SIM, 133);
