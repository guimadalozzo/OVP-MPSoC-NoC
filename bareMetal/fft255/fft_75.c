...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 151);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_75: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_75 %d", logN);
		...
		printf("Recebeu imag_75");
		printf("Inicio da FFT_75");

		FFT(logN, real, im);

		printf("Fim da FFT_75");
		...
		printf("Envio completo do Frame_75");
	}

	printf("Fim da Aplicacao FFT_75");
	MemoryWrite(END_SIM, 151);
