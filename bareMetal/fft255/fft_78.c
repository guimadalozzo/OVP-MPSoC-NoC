...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 157);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_78: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_78 %d", logN);
		...
		printf("Recebeu imag_78");
		printf("Inicio da FFT_78");

		FFT(logN, real, im);

		printf("Fim da FFT_78");
		...
		printf("Envio completo do Frame_78");
	}

	printf("Fim da Aplicacao FFT_78");
	MemoryWrite(END_SIM, 157);
