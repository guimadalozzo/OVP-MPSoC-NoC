...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 167);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_83: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_83 %d", logN);
		...
		printf("Recebeu imag_83");
		printf("Inicio da FFT_83");

		FFT(logN, real, im);

		printf("Fim da FFT_83");
		...
		printf("Envio completo do Frame_83");
	}

	printf("Fim da Aplicacao FFT_83");
	MemoryWrite(END_SIM, 167);
