...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 177);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_88: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_88 %d", logN);
		...
		printf("Recebeu imag_88");
		printf("Inicio da FFT_88");

		FFT(logN, real, im);

		printf("Fim da FFT_88");
		...
		printf("Envio completo do Frame_88");
	}

	printf("Fim da Aplicacao FFT_88");
	MemoryWrite(END_SIM, 177);
