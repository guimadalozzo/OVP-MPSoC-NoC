...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 179);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_89: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_89 %d", logN);
		...
		printf("Recebeu imag_89");
		printf("Inicio da FFT_89");

		FFT(logN, real, im);

		printf("Fim da FFT_89");
		...
		printf("Envio completo do Frame_89");
	}

	printf("Fim da Aplicacao FFT_89");
	MemoryWrite(END_SIM, 179);
