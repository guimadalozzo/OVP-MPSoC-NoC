...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 185);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_92: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_92 %d", logN);
		...
		printf("Recebeu imag_92");
		printf("Inicio da FFT_92");

		FFT(logN, real, im);

		printf("Fim da FFT_92");
		...
		printf("Envio completo do Frame_92");
	}

	printf("Fim da Aplicacao FFT_92");
	MemoryWrite(END_SIM, 185);
