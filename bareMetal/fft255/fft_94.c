...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 189);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_94: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_94 %d", logN);
		...
		printf("Recebeu imag_94");
		printf("Inicio da FFT_94");

		FFT(logN, real, im);

		printf("Fim da FFT_94");
		...
		printf("Envio completo do Frame_94");
	}

	printf("Fim da Aplicacao FFT_94");
	MemoryWrite(END_SIM, 189);
