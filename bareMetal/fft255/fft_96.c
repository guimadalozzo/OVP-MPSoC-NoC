...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 193);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_96: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_96 %d", logN);
		...
		printf("Recebeu imag_96");
		printf("Inicio da FFT_96");

		FFT(logN, real, im);

		printf("Fim da FFT_96");
		...
		printf("Envio completo do Frame_96");
	}

	printf("Fim da Aplicacao FFT_96");
	MemoryWrite(END_SIM, 193);
