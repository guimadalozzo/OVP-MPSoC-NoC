...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 195);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_97: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_97 %d", logN);
		...
		printf("Recebeu imag_97");
		printf("Inicio da FFT_97");

		FFT(logN, real, im);

		printf("Fim da FFT_97");
		...
		printf("Envio completo do Frame_97");
	}

	printf("Fim da Aplicacao FFT_97");
	MemoryWrite(END_SIM, 195);
