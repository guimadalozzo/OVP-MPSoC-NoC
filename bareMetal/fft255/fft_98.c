...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 197);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_98: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_98 %d", logN);
		...
		printf("Recebeu imag_98");
		printf("Inicio da FFT_98");

		FFT(logN, real, im);

		printf("Fim da FFT_98");
		...
		printf("Envio completo do Frame_98");
	}

	printf("Fim da Aplicacao FFT_98");
	MemoryWrite(END_SIM, 197);
