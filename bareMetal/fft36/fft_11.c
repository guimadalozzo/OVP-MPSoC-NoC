...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 23);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_11: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_11 %d", logN);
		...
		printf("Recebeu imag_11");
		printf("Inicio da FFT_11");

		FFT(logN, real, im);

		printf("Fim da FFT_11");
		...
		printf("Envio completo do Frame_11");
	}

	printf("Fim da Aplicacao FFT_11");
	MemoryWrite(END_SIM, 23);
