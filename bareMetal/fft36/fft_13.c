...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 27);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_13: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_13 %d", logN);
		...
		printf("Recebeu imag_13");
		printf("Inicio da FFT_13");

		FFT(logN, real, im);

		printf("Fim da FFT_13");
		...
		printf("Envio completo do Frame_13");
	}

	printf("Fim da Aplicacao FFT_13");
	MemoryWrite(END_SIM, 27);
