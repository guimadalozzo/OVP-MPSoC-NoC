...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 29);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_14: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_14 %d", logN);
		...
		printf("Recebeu imag_14");
		printf("Inicio da FFT_14");

		FFT(logN, real, im);

		printf("Fim da FFT_14");
		...
		printf("Envio completo do Frame_14");
	}

	printf("Fim da Aplicacao FFT_14");
	MemoryWrite(END_SIM, 29);
