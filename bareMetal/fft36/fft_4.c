...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 9);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_4: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_4 %d", logN);
		...
		printf("Recebeu imag_4");
		printf("Inicio da FFT_4");
		...
		printf("Envio completo do Frame_4");
	}

	printf("Fim da Aplicacao FFT_4");
	MemoryWrite(END_SIM, 9);
