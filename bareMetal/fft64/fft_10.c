...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 21);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_10: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_10 %d", logN);
		...
		printf("Recebeu imag_10");
		printf("Inicio da FFT_10");

		FFT(logN, real, im);

		printf("Fim da FFT_10");
		...
		printf("Envio completo do Frame_10");
	}

	printf("Fim da Aplicacao FFT_10");
	MemoryWrite(END_SIM, 21);
