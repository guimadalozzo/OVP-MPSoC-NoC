...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 33);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_16: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_16 %d", logN);
		...
		printf("Recebeu imag_16");
		printf("Inicio da FFT_16");

		FFT(logN, real, im);

		printf("Fim da FFT_16");
		...
		printf("Envio completo do Frame_16");
	}

	printf("Fim da Aplicacao FFT_16");
	MemoryWrite(END_SIM, 33);
