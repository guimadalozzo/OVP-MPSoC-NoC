...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 45);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_22: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_22 %d", logN);
		...
		printf("Recebeu imag_22");
		printf("Inicio da FFT_22");

		FFT(logN, real, im);

		printf("Fim da FFT_22");
		...
		printf("Envio completo do Frame_22");
	}

	printf("Fim da Aplicacao FFT_22");
	MemoryWrite(END_SIM, 45);
