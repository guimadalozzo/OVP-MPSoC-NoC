...
int main() {
	MemoryWrite(INITIALIZE_ROUTER, 49);
	unsigned int logN;
	...
	while(1) {
		...
		printf("Inicio do processamento do frame_24: %d", i++);

		msg = Receive();
		logN = msg.msg[0];

		printf("Recebeu logN_24 %d", logN);
		...
		printf("Recebeu imag_24");
		printf("Inicio da FFT_24");

		FFT(logN, real, im);

		printf("Fim da FFT_24");
		...
		printf("Envio completo do Frame_24");
	}

	printf("Fim da Aplicacao FFT_24");
	MemoryWrite(END_SIM, 49);
